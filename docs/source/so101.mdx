...
In the steps below, we explain how to assemble our flagship robot, the SO-101-6DOF.

## Source the parts

Follow this [README](https://github.com/EmbodiedAI-Group/SO-ARM100-6DOF). It contains the bill of materials, with a link to source the parts, as well as the instructions to 3D print the parts.
...
The follower arm uses 7x STS3215 motors with 1/345 gearing. The leader, however, uses three differently geared motors to make sure it can both sustain its own weight and it can be moved without requiring much force. Which motor is needed for which joint is shown in the table below.
